...
public class PassengerSeatPlan {
	...
	private FlightAircraftService fs;
	...
	public PassengerSeatPlan(FlightId flightId) {
		...
		fs = new FlightAircraftService(new FlightAircraftDAImpl());
		this.layout = fs.getAircraftLayout(flightId.getFlightNo());
		this.occupiedSeats = fs.getAllOccupiedSeatsByFlight(this.flightId);
	}
	...
	private void createSeatPlanHTML() {
		this.firstClassHTML = createAreaHTML(FIRSTCLASS, this.layout.getFirstClassRows(), this.layout.getFirstClassSeatArrangement());
		this.businessClassHTML = createAreaHTML(BUSINESSCLASS, this.layout.getBusinessClassRows(), this.layout.getBusinessClassSeatArrangement());
		this.economyClassHTML = createAreaHTML(ECONOMYCLASS, this.layout.getEconomyClassRows(), this.layout.getEconomyClassSeatArrangement());
		this.rowCounter = 0;
